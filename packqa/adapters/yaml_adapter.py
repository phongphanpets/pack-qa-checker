...
_PRODUCT_FIELDS: dict[str, Callable[[Any], Any]] = {
    "name": str,
    "price": float,
    "full_price": float,
    "currency": str,
    "enabled": lambda value: _as_bool(value),
    "hidden": lambda value: _as_bool(value),
    "test_mode": lambda value: _as_bool(value),
    "starts_at": lambda value: _as_datetime(value),
    "ends_at": lambda value: _as_datetime(value),
    "reset_interval": str,
    "next_reset_at": lambda value: _as_datetime(value),
}
...
def _bundle(raw: Any, locator: str) -> PackBundle:
    ...
    product = (
        _product_listing(raw["product"], f"{locator}.product")
        if isinstance(raw.get("product"), Mapping)
        else None
    )
    return PackBundle(bundle_id=bundle_id, product=product, **packs)
...
def _product_listing(raw: Mapping[str, Any], locator: str) -> ProductListing:
    try:
        source = Source(raw.get("source", Source.ADMIN))
    except ValueError as error:
        raise YamlAdapterError(f"{locator}: invalid source") from error

    product_uuid = _field(
        raw.get("product_uuid"),
        source,
        str,
        f"{locator}.product_uuid",
    )
    if not product_uuid.is_verifiable:
        raise YamlAdapterError(f"{locator}.product_uuid is required")

    fields = {
        name: _field(raw.get(name), source, converter, f"{locator}.{name}")
        for name, converter in _PRODUCT_FIELDS.items()
    }
    raw_limits = raw.get("purchase_limits", raw.get("limits", {}))
    if raw_limits is None:
        raw_limits = {}
    if not isinstance(raw_limits, Mapping):
        raise YamlAdapterError(f"{locator}.purchase_limits must be a mapping")
    limits = PurchaseLimits(
        **{
            name: _field(
                ...
                source,
                int,
                f"{locator}.purchase_limits.{name}",
            )
            for name in ("player", "server", "character", "product")
        }
    )
    raw_links = raw.get("bundle_links", [])
    if raw_links is None:
        raw_links = []
    if not isinstance(raw_links, list):
        raise YamlAdapterError(f"{locator}.bundle_links must be a list")
    links = [
        _product_bundle_link(item, source, f"{locator}.bundle_links[{index}]")
...
def _product_bundle_link(
    raw: Any,
    source: Source,
    locator: str,
) -> ProductBundleLink:
    if not isinstance(raw, Mapping):
        raise YamlAdapterError(f"{locator}: bundle link must be a mapping")
    bundle_id = _field(raw.get("bundle_id"), source, int, f"{locator}.bundle_id")
    if not bundle_id.is_verifiable:
        raise YamlAdapterError(f"{locator}.bundle_id is required")
    return ProductBundleLink(
        bundle_id=bundle_id,
        is_primary=_field(
            raw.get("is_primary"),
            source,
            _as_bool,
            f"{locator}.is_primary",
        ),
    )
...
def _as_datetime(value: Any) -> datetime:
    ...
    if parsed.tzinfo is None:
        raise ValueError("datetime requires timezone information")
    return parsed
