...
from packqa.request_hub import RequestStore, notify_discord
...
def make_server(
    ...
    *,
    ...
    request_history_path: str | Path = "data/import_requests.sqlite3",
) -> ThreadingHTTPServer:
    engine = RuleEngine.from_yaml(rules_path)
    history = HistoryStore(history_path)
    requests = RequestStore(request_history_path)
    ...
    class ValidationHandler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:
            ...
            if parsed.path == "/api/requests":
                ...
                try:
                    ...
                    limit = 100
                self._json(200, {"requests": requests.list(limit)})
                return
            request_id = _request_id(parsed.path)
            if request_id:
                entry = requests.get(request_id)
                if entry is None:
                    self._json(404, {"error": "Request not found"})
                else:
                    self._json(200, {"request": entry})
                return
        ...
        def do_POST(self) -> None:
            ...
            if parsed.path == "/api/requests":
                try:
                    entry = requests.create(self._payload())
                    notification = notify_discord(entry, event="New import request")
                    requests.mark_notification(entry["id"], notification)
                    entry["notification_status"] = notification
                    self._json(201, {"request": entry})
                except (json.JSONDecodeError, TypeError, ValueError) as error:
                    self._json(400, {"error": str(error)})
                return
            request_status_id = _request_status_id(parsed.path)
            if request_status_id:
                try:
                    payload = self._payload()
                    entry = requests.update_status(request_status_id, str(payload.get("status") or ""))
                    if entry is None:
                        self._json(404, {"error": "Request not found"})
                        return
                    notification = notify_discord(entry, event="Import request status updated")
                    requests.mark_notification(entry["id"], notification)
                    entry["notification_status"] = notification
                    self._json(200, {"request": entry})
                except (json.JSONDecodeError, TypeError, ValueError) as error:
            ...
            try:
                ...
                aztek_json = payload.get("aztek_dom_observations")
                if aztek_json is not None:
                    bundles = apply_aztek_dom_observations(
                        bundles,
                        AztekDomObservationDocument.model_validate(aztek_json),
                        _aztek_dom_targets_from_json(payload.get("aztek_dom_targets")),
                    )
...
def _request_status_id(path: str) -> str | None:
    prefix = "/api/requests/"
    suffix = "/status"
...
def _aztek_dom_targets_from_json(raw: Any) -> list[AztekDomTarget]:
    if not isinstance(raw, list):
        raise ValueError(
            "aztek_dom_targets must be a list when aztek_dom_observations is provided"
        )
    try:
        return [AztekDomTarget.model_validate(item) for item in raw]
    except (TypeError, ValueError) as error:
        raise ValueError("invalid aztek_dom_targets") from error
