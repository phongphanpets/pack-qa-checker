...
export default function ExcelPasteForm({
  ...
  const totalItems = result.bundles.reduce(
      ...
      {!value ? (
        ...
        <div className="excel-paste-result">
            ...
            <div>
              ...
              <small>
                {result.bundles.length > 1
                  ? `อ่านได้ ${result.bundles.length} bundles · ${totalItems} items`
                  : `อ่านได้ ${result.summary.itemCount} items · Seed Point ${result.summary.seedPoint ?? "—"} · GSP ${result.summary.gspEarn ?? "—"} · Limit ${result.summary.purchaseLimit ?? "—"}`}
              </small>
            </div>
          </div>
          ...
          <div className="excel-paste-tags">
            {result.bundles.length > 1 && (
              <span>{result.bundles.length} Bundles ใน Request เดียว</span>
            )}
          ...
          {result.warnings.map((warning, index) => (
            <p className="excel-paste-warning" key={`${warning.code}-${index}`}>
              <span>!</span>
              {warning.message}
            </p>
          ))}

          <ItemCatalogCheck bundles={result.bundles} />

          <details className="excel-paste-preview">
            <summary>ดูรายการที่อ่านได้</summary>
            <div>
              {result.bundles.map((bundle) => (
                <section key={bundle.bundle_id} className="excel-paste-bundle-preview">
                  {result.bundles.length > 1 && <strong>{bundle.name || "ไม่มีชื่อ Bundle"}</strong>}
                  {bundle.items.map((item) => (
                    <p key={`${bundle.bundle_id}-${item.item_id}-${item.name}-${item.amount}`}>
