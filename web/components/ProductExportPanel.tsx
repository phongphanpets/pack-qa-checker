...
type ProductLine = { key: string; bundleName: string; name: string; price: string; saleStart: string; saleEnd: string };
...
export default function ProductExportPanel({ productName, bundles, selectedIndexes, fallbackPrice, fallbackLimit }: Props) {
  ...
  async function download() {
    ...
    try {
      const baseOrder = Number(displayOrder) || 0;
      const drafts: LocalProductDraft[] = lines.map((line, index) => ({
        name: line.name, category, displayOrder: String(baseOrder + index), saleStart: line.saleStart, saleEnd: line.saleEnd,
