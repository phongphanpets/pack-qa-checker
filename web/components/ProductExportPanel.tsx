...
export default function ProductExportPanel({ productName, bundles, selectedIndexes, fallbackPrice, fallbackLimit, manual = false }: Props) {
  ...
  function resizeProducts() {
    ...
    setExportError("");
    setLines(current => Array.from({ length: size }, (_, index) => current[index] || { key: `manual:${crypto.randomUUID()}`, bundleName: "", bundleNames: [], name: "", price: "", tags: [], purchaseLimit: "1", displayOrder: String((Number(displayOrder) || 0) - index), saleStart: commonStart, saleEnd: commonEnd }));
  }
  ...
  async function download() {
    ...
    try {
      const drafts: LocalProductDraft[] = lines.map((line) => ({
        ...
        tags: line.tags, purchaseLimit: line.purchaseLimit, currency, actualPrice: line.price, fullPrice: line.price, bundleNames: line.bundleNames,
      }));
...
const productTags = ["Rank Bronze", "Sale", "Limited", "Flashsale"];
function TagPicker({ label, value, onChange }: { label: string; value: string[]; onChange: (tags: string[]) => void }) {
  return <fieldset className="product-tags"><legend>{label}</legend><div>{productTags.map(tag => <label key={tag}><input type="checkbox" checked={value.includes(tag)} onChange={event => onChange(event.target.checked ? [...value, tag] : value.filter(item => item !== tag))} />{tag}</label>)}</div></fieldset>;
}
