...
$webUpstreamUrl = "http://127.0.0.1:3005"
...
if (-not (Test-PackQaEndpoint -Url $webUpstreamUrl)) {
    Write-Host "Preparing Pack QA web preview..." -ForegroundColor Cyan
    & $npmCommand.Source run build
    if ($LASTEXITCODE -ne 0) {
        Stop-WithMessage "Pack QA web build could not be prepared."
    }

    $webUpstreamOutLog = Join-Path $runtimeRoot "web-upstream.log"
    $webUpstreamErrorLog = Join-Path $runtimeRoot "web-upstream-error.log"
    Start-Process `
        -FilePath $npmCommand.Source `
        -ArgumentList @("run", "start", "--", "--port", "3005") `
        -WorkingDirectory $webRoot `
        -WindowStyle Hidden `
        -RedirectStandardOutput $webUpstreamOutLog `
        -RedirectStandardError $webUpstreamErrorLog | Out-Null
}

if (-not (Wait-PackQaEndpoint -Url $webUpstreamUrl -Seconds 45)) {
    Stop-WithMessage "Pack QA web preview could not start."
}
...
if (-not (Test-PackQaEndpoint -Url $webUrl)) {
    ...
    $previousUpstreamPort = $env:PACK_QA_UPSTREAM_PORT
    $env:PORT = "3000"
    $env:PACK_QA_UPSTREAM_PORT = "3005"
    Start-Process `
        -FilePath $npmCommand.Source `
        -ArgumentList @("run", "preview:windows") `
        -WorkingDirectory $webRoot `
        -WindowStyle Hidden `
        -RedirectStandardOutput $webOutLog `
        -RedirectStandardError $webErrorLog | Out-Null
    $env:PORT = $previousPort
    $env:PACK_QA_UPSTREAM_PORT = $previousUpstreamPort
}
