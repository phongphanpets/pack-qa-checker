...
def test_local_api_applies_aztek_dom_readback_by_uuid_and_bundle_id() -> None:
...
""",
                "aztek_dom_targets": [
                    {"product_uuid": product_uuid, "bundle_id": 115184}
                ],
                "aztek_dom_observations": {
                    "products": [
                        {
                            ...
                            "bundles": [
                                {"bundle_id": 115184, "bundle_type": {"value": "FIXED"}}
                            ],
                        }
                    ]
                },
        ...
        admin_name = next(
            finding
            for finding in report["bundles"][0]["findings"]
            if finding["rule_id"] == "PACK_NAME_ADMIN"
        )
        assert admin_name["status"] == "PASS"
        assert admin_name["actual_locator"] == "label:ชื่อสินค้า (ไทย) *"
    finally:
        server.shutdown()
        server.server_close()
        thread.join(timeout=5)
