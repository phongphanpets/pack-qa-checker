...
- A separate Vite client entry is in web/pages; build with `npx vite build --config vite.pages.config.ts` from web. Existing vinext/server configuration remains available.
...
- Bundled quick_validate.py could not run because the available Python yaml module lacks safe_load; frontmatter and references still need validation with a functioning YAML parser.
